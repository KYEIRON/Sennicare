...
describe('operating time zone', () => {
  it('renders a date in Eastern time', () => {
    // 2025-05-20 18:30 UTC = 2:30 PM EDT
    const instant = new Date('2025-05-20T18:30:00Z');
    expect(formatOperatingDate(instant)).toBe('Tuesday, May 20, 2025');
    expect(formatOperatingTime(instant)).toBe('2:30 PM EDT');
  });

  it('uses the US business day, not the UK one', () => {
    // 2025-05-21 02:00 UTC is still 2025-05-20 in North Carolina (10 PM EDT),
    // while in the United Kingdom it is already the 21st.
    const instant = new Date('2025-05-21T02:00:00Z');
    expect(operatingDateKey(instant)).toBe('2025-05-20');
    expect(operatingHour(instant)).toBe(22);
  });

  it('handles the winter offset as well as the summer one', () => {
    // 2025-01-15 18:30 UTC = 1:30 PM EST
    const instant = new Date('2025-01-15T18:30:00Z');
